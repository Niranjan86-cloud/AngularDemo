...
dataList: Array<any> = [];
 ...
 get total() {
  return this.items.reduce(
    (sum, x) => ({
      qtyTotal: 1,
      discounted_price: sum.discounted_price + x.qtyTotal * x.discounted_price
    }),
    { qtyTotal: 1, discounted_price: 0 }
  ).discounted_price;
}

changeSubtotal(item: { qtyTotal: any; discounted_price: any; }, index: string | number) {
  const qty = item.qtyTotal;
  const amt = item.discounted_price;
  ...
  this.mysharedService.saveCart();
}

//----- remove specific item
removeFromCart(item: any) {
  this.mysharedService.removeItem(item);
  this.items = this.mysharedService.getItems();
}

//----- clear cart item
clearCart(items: any) {
  // this.items.forEach((item, index) => this.mysharedService.removeItem(index));
  this.mysharedService.clearCart(items);
  this.items = [...this.mysharedService.getItems()];
}

//----- add item to cart
addToCart(item: { qtyTotal: number; }) {
  if (!this.mysharedService.itemInCart(item)) {
    item.qtyTotal = 1;
    this.mysharedService.addToCart(item); //add items in cart
    this.items = [...this.mysharedService.getItems()];
  }
}

  ngOnInit(): void {
    this.mysharedService.loadCart();
    this.items = this.mysharedService.getItems();
  }
